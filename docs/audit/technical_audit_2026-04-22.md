...
- **P0** : unifier le schéma métier (instructor), corriger relations Eloquent, seeders, redirections front.
- **P1** : fermer le scope MVP (cours -> leçon -> quiz -> progression), supprimer/masquer les écrans non prêts.
- **P2** : tests E2E happy paths + erreurs, hardening auth/session/cookies/CORS.
- **P3** : CI/CD minimal, observabilité, checklists prod, déploiement staging puis prod.
