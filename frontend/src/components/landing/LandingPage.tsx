...
function SectionHeader({ kicker, title, description }: { kicker: string; title: string; description: string }) {
  return (
    <div className="mx-auto max-w-3xl text-center">
      <p className="text-sm font-semibold uppercase tracking-[0.22em] text-[var(--primary)]">{kicker}</p>
      <h2 className="mt-3 text-3xl font-semibold tracking-[-0.03em] text-[var(--foreground)] md:text-5xl">{title}</h2>
      <p className="mt-5 text-base leading-7 text-[var(--muted)] md:text-lg">{description}</p>
    </div>
  );
}
...
function InterfaceMockup() {
  return (
    <div className="relative mx-auto w-full max-w-xl rounded-[2rem] border border-[var(--border)] bg-white/85 p-3 shadow-2xl shadow-[var(--primary)]/10 backdrop-blur">
      <div className="rounded-[1.5rem] border border-slate-100 bg-[linear-gradient(135deg,#f8fbff_0%,#ffffff_48%,#eef6ff_100%)] p-4">
        <div className="flex items-center justify-between border-b border-slate-200 pb-4">
          <div>
            <p className="text-xs font-semibold uppercase tracking-[0.18em] text-[var(--primary)]">Classe live</p>
            <p className="mt-1 text-lg font-semibold text-[var(--foreground)]">Data Foundations · Cohorte 24</p>
          </div>
        ...
        <div className="mt-4 grid gap-3 md:grid-cols-[1.2fr_0.8fr]">
          ...
          <div className="space-y-3">
            <div className="rounded-2xl border border-slate-200 bg-white p-4">
              <p className="text-xs font-semibold uppercase tracking-[0.16em] text-[var(--muted)]">Progression</p>
              <p className="mt-2 text-3xl font-semibold text-[var(--foreground)]">78%</p>
              <div className="mt-3 h-2 rounded-full bg-slate-100">
                <div className="h-2 w-[78%] rounded-full bg-[var(--primary)]" />
              </div>
            </div>
            <div className="rounded-2xl border border-slate-200 bg-white p-4">
              <p className="text-sm font-semibold text-[var(--foreground)]">Certification</p>
              <p className="mt-1 text-sm text-[var(--muted)]">PDF généré après réussite des modules obligatoires.</p>
            </div>
          </div>
        </div>
      </div>
    ...
    <section className="relative overflow-hidden px-4 pb-16 pt-12 md:px-6 md:pb-24 md:pt-20" id="hero">
      ...
      <div className="mx-auto grid max-w-7xl items-center gap-12 lg:grid-cols-[0.95fr_1.05fr]">
        <div className="max-w-3xl">
          <div className="inline-flex items-center gap-2 rounded-full border border-[var(--border)] bg-white px-4 py-2 text-sm font-medium text-[var(--muted)] shadow-sm">
            <span className="h-2 w-2 rounded-full bg-emerald-400" />
            Nouvelle génération d’apprentissage certifiant
          </div>
          <h1 className="mt-6 text-5xl font-semibold tracking-[-0.055em] text-[var(--foreground)] md:text-7xl">
...
function CredibilitySection() {
  return (
    <section className="border-y border-[var(--border)] bg-white/75 px-4 py-10 md:px-6" id="credibilite">
      <div className="mx-auto max-w-7xl">
        ...
        <div className="mt-8 grid grid-cols-2 gap-3 md:grid-cols-5">
          {partners.map((partner) => (
            <div
              className="flex h-20 items-center justify-center rounded-2xl border border-[var(--border)] bg-white px-4 text-center text-sm font-semibold text-slate-500 shadow-sm"
              key={partner}
            >
              {partner}
            </div>
  ...
  return (
    <section className="px-4 py-20 md:px-6" id="positionnement">
      <div className="mx-auto grid max-w-7xl gap-6 lg:grid-cols-[0.9fr_1.1fr]">
        <div className="rounded-[2rem] border border-[var(--border)] bg-white p-8 shadow-sm md:p-10">
          <p className="text-sm font-semibold uppercase tracking-[0.22em] text-[var(--primary)]">Étape 1 · L’esprit Ed-tech</p>
          <h2 className="mt-4 text-3xl font-semibold tracking-[-0.035em] text-[var(--foreground)] md:text-5xl">
            Une UVP simple : apprendre avec exigence, prouver avec confiance.
          </h2>
        </div>
        <div className="grid gap-4 md:grid-cols-2">
          <div className="rounded-[2rem] border border-[var(--border)] bg-[var(--secondary)] p-7">
            <h3 className="text-xl font-semibold text-[var(--foreground)]">Pour l’étudiant</h3>
          ...
          <div className="rounded-[2rem] border border-[var(--border)] bg-white p-7 shadow-sm">
            <h3 className="text-xl font-semibold text-[var(--foreground)]">Pour le professionnel</h3>
            <p className="mt-3 leading-7 text-[var(--muted)]">
          ...
          <div className="rounded-[2rem] border border-sky-100 bg-[linear-gradient(135deg,#ffffff,#eef7ff)] p-7 md:col-span-2">
            <h3 className="text-xl font-semibold text-[var(--foreground)]">Pourquoi le style Stripe/Coursera inspire confiance</h3>
            <p className="mt-3 leading-7 text-[var(--muted)]">
...
function LiveSection() {
  return (
    <section className="bg-[var(--foreground)] px-4 py-20 text-white md:px-6" id="live">
      ...
      <div className="mx-auto max-w-7xl">
        ...
        <div className="mt-12 grid gap-5 lg:grid-cols-3">
          {ecosystem.map((item, index) => (
            <article
              className={`rounded-[2rem] border border-[var(--border)] bg-white p-7 shadow-sm ${
                index === 0 ? 'lg:row-span-2' : ''
              }`}
              key={item.title}
            >
              <p className="text-sm font-semibold uppercase tracking-[0.18em] text-[var(--primary)]">{item.eyebrow}</p>
              <h3 className="mt-4 text-2xl font-semibold tracking-[-0.025em] text-[var(--foreground)]">{item.title}</h3>
              <p className="mt-4 leading-7 text-[var(--muted)]">{item.text}</p>
              <div className="mt-8 rounded-2xl bg-[var(--secondary)] p-5">
                <p className="text-4xl font-semibold text-[var(--foreground)]">{item.metric}</p>
                <p className="mt-2 text-sm leading-6 text-[var(--muted)]">{item.metricLabel}</p>
              </div>
            </article>
          ))}
        </div>
      ...
      <div className="mx-auto max-w-7xl">
        ...
        <div className="mt-12 grid gap-5 md:grid-cols-2">
          {testimonials.map((testimonial) => (
            <figure className="rounded-[2rem] border border-[var(--border)] bg-[var(--secondary)] p-8" key={testimonial.name}>
              <blockquote className="text-xl leading-9 tracking-[-0.015em] text-[var(--foreground)]">“{testimonial.quote}”</blockquote>
...
function TechnicalSection() {
  return (
    <section className="px-4 py-20 md:px-6" id="architecture">
      <div className="mx-auto max-w-7xl rounded-[2rem] border border-[var(--border)] bg-white p-8 shadow-sm md:p-10">
        <p className="text-sm font-semibold uppercase tracking-[0.22em] text-[var(--primary)]">Étape 3 · Architecture UI</p>
        <div className="mt-6 grid gap-8 lg:grid-cols-[0.9fr_1.1fr]">
          <div>
            <h2 className="text-3xl font-semibold tracking-[-0.035em] text-[var(--foreground)] md:text-5xl">
          ...
          <div className="grid gap-3 sm:grid-cols-2">
            {[
              ...
              <div className="rounded-2xl border border-[var(--border)] p-4" key={name}>
                <div className="h-16 rounded-xl border border-slate-200" style={{ backgroundColor: value }} />
                <p className="mt-3 font-semibold text-[var(--foreground)]">{name}</p>
...
function FinalCtaSection() {
  return (
    <section className="px-4 pb-24 pt-6 md:px-6" id="inscription">
      <div className="mx-auto max-w-6xl overflow-hidden rounded-[2rem] border border-sky-100 bg-[radial-gradient(circle_at_10%_10%,rgba(125,211,252,.35),transparent_24%),linear-gradient(135deg,#ffffff_0%,#eef7ff_52%,#f8fbff_100%)] p-8 text-center shadow-2xl shadow-[var(--primary)]/10 md:p-14">
        <p className="text-sm font-semibold uppercase tracking-[0.22em] text-[var(--primary)]">Inscription sécurisée via Auth Sanctum</p>
        <h2 className="mx-auto mt-4 max-w-3xl text-3xl font-semibold tracking-[-0.04em] text-[var(--foreground)] md:text-6xl">
